...
import { EzriRealtimeClient, type EzriAvatarData, type EzriWsStatus } from "@/lib/ezri/realtimeClient";
...
type JordanMorphName = (typeof JORDAN_MORPH_NAMES)[number];

const JORDAN_MORPH_NAME_SET = new Set<string>(JORDAN_MORPH_NAMES);

const JORDAN_VISEME_NAMES: JordanMorphName[] = [
  "viseme_rest",
  "viseme_AA",
  "viseme_IH",
  "viseme_O",
...
const JORDAN_BLINK_MORPH_NAMES: JordanMorphName[] = [
  "eyeBlinkLeft",
  "eyeBlinkRight",
  "eye_blink",
];
...
const JORDAN_RFV2_LOOK_AHEAD_SECONDS = 0.06;
const JORDAN_RFV2_FACE_TUNING = {
  speechLerpSpeed: 15,
  decayLerpSpeed: 8.5,
  jawLerpSpeed: 7.5,
...
function isRfv2VisemeMorphName(name: string): boolean {
  return JORDAN_VISEME_NAMES.includes(name as JordanMorphName);
}

function isRfv2BlinkMorphName(name: string): boolean {
  return JORDAN_BLINK_MORPH_NAMES.includes(name as JordanMorphName);
}

function isRfv2ExpressionMorphName(name: string): boolean {
...
const JORDAN_HEAD_OFFSET_Y = 0.44;// move head up/down shaz
const JORDAN_HEAD_OFFSET_X = -0.01;// move head left/right shaz
...
type AvatarPhonemeTimeline = {
  sentence: string;
  sentiment: string;
  phonemes: AvatarPhoneme[];
};

type AvatarRenderMode = "legacyHybrid" | "rfv2Morph";
...
const EMPTY_JORDAN_PHONEME_DEBUG_STATE: JordanPhonemeDebugState = {
  audioCurrentTime: 0,
  lookAheadMs: JORDAN_RFV2_LOOK_AHEAD_SECONDS * 1000,
...
function normalizePhonemeLabel(value: unknown): string {
...
function normalizePhonemeLabelForDebug(value: unknown): string {
  const raw = String(value ?? "");
  if (!raw.trim()) return "REST";
  let label = raw.trim().toUpperCase();
  label = label.replace(/^VISEME[_\s-]*/i, "");
...
function normalizeAvatarPhonemeTimeline(
  ...
  source.forEach((item, index) => {
    ...
    const rawLabel = obj.phoneme ?? obj.phone ?? obj.label ?? obj.name ?? obj.value;
    ...
    const end =
      ...
      asNumber(obj.stop) ??
      asNumber(obj.offset);
    const duration = asNumber(obj.duration) ?? asNumber(obj.dur);
    const fallbackStart =
      durationSeconds && source.length > 0
...
function findActiveJordanViseme(
...
function findActiveJordanPhoneme(
  timeline: AvatarPhonemeTimeline | null,
  speechTime: number
...
function hasInvalidJordanPhonemeTimestamps(
  timeline: AvatarPhonemeTimeline | null
): boolean {
...
function isSadSentiment(sentiment: string): boolean {
...
function jordanJawSupportForViseme(viseme: JordanMorphName | null): number {
  switch (viseme) {
    case "viseme_AA":
      return 0.22;
    case "viseme_O":
...
function isJordanBlinkMorph(name: JordanMorphName): boolean {
  return JORDAN_BLINK_MORPH_NAMES.includes(name);
}

function getJordanBlinkMode(bindings: Map<JordanMorphName, MorphBinding[]>): string {
  const hasSplitBlink =
...
function ThreeAvatar({
  ...
  /** RFv2/Jordan: exact morph targets only, no facial bone mouth fallback. */
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        if (useRfv2Morphs) {
          ...
          // Center and frame the full GLB hierarchy for legacy avatars. Morph
          // discovery only caches references; it must never decide rendering.
        ...
        if (process.env.NODE_ENV === "development") {
          ...
          if (useRfv2Morphs) {
            ...
            console.log("[Avatar] Jordan lookAheadMs:", JORDAN_RFV2_LOOK_AHEAD_SECONDS * 1000);
            console.log(
    ...
    const renderLoop = () => {
      ...
      if (useRfv2Morphs) {
        ...
        const speechTime = audioCurrentTime + JORDAN_RFV2_LOOK_AHEAD_SECONDS;
        ...
        targets.set("mouthSmileLeft", smileOverlay * 0.96);
        targets.set("mouthSmileRight", smileOverlay * 1.04);
        targets.set("mouthFrownLeft", frownOverlay * 1.03);
        targets.set("mouthFrownRight", frownOverlay * 0.97);
        targets.set("cheekSquintLeft", cheekOverlay * 0.94);
        targets.set("cheekSquintRight", cheekOverlay * 1.06);
        targets.set(
          "eyebrows",
          browOverlay
        );
        ...
        const jawSupport = speaking
          ...
          : 0;
        targets.set("jawOpen", jawSupport);

        const gazeWaveA = Math.sin(now * 0.00055);
        const gazeWaveB = Math.sin(now * 0.00037 + 1.8);
        const gazeUp = THREE.MathUtils.clamp(Math.max(0, gazeWaveA) * 0.035, 0, 0.055);
        const gazeDown = THREE.MathUtils.clamp(Math.max(0, -gazeWaveA) * 0.03, 0, 0.05);
        const gazeAsym = gazeWaveB * 0.006;
        targets.set("eyeLookUpLeft", THREE.MathUtils.clamp(gazeUp + gazeAsym, 0, 0.12));
        targets.set("eyeLookUpRight", THREE.MathUtils.clamp(gazeUp - gazeAsym, 0, 0.12));
        targets.set("eyeLookDownLeft", THREE.MathUtils.clamp(gazeDown - gazeAsym, 0, 0.12));
        targets.set("eyeLookDownRight", THREE.MathUtils.clamp(gazeDown + gazeAsym, 0, 0.12));
        ...
        const emotionAlpha = 1 - Math.exp(-JORDAN_RFV2_FACE_TUNING.emotionLerpSpeed * dt);
        const gazeAlpha = 1 - Math.exp(-JORDAN_RFV2_FACE_TUNING.gazeLerpSpeed * dt);
        JORDAN_MORPH_NAMES.forEach((name) => {
          if (isJordanBlinkMorph(name)) return;
    ...
    function animateBlink(duration = 320, onDone?: () => void) {
      ...
      const rfv2PartialBlink = useRfv2Morphs && Math.random() < 0.18;
      const rfv2LeftMax = rfv2PartialBlink
        ? 0.38 + Math.random() * 0.12
        : 0.84 + Math.random() * 0.06;
      const rfv2RightMax = THREE.MathUtils.clamp(
        rfv2LeftMax + (Math.random() - 0.5) * 0.08,
        rfv2PartialBlink ? 0.32 : 0.78,
        rfv2PartialBlink ? 0.56 : 0.92
      );
      const rfv2RightLagMs = rfv2HasSplitBlink ? (Math.random() - 0.5) * 18 : 0;
      ...
      const tick = (now: number) => {
        ...
        const blinkAt = (elapsedMs: number) => {
          ...
          const tri = localT < 0.5 ? localT * 2 : (1 - localT) * 2; // 0..1..0
          return smoothstep(tri);
        };
        const blinkValue = blinkAt(now - start);
        const rfv2LeftBlinkValue = blinkAt(now - start);
        const rfv2RightBlinkValue = blinkAt(now - start - rfv2RightLagMs);
        ...
        blinkBindingsRef.current.forEach(
          ...
          if (useRfv2Morphs) {
            ...
            const rfv2MaxBlink =
              lower === "eyeblinkright" ? rfv2RightMax : rfv2LeftMax;
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {process.env.NODE_ENV === "development" &&
        ...
        useRfv2Morphs && (
          <div className="pointer-events-none absolute left-3 top-3 z-30 max-w-[280px] rounded-md border border-white/15 bg-black/75 px-3 py-2 font-mono text-[10px] leading-relaxed text-white shadow-lg">
            <div className="mb-1 text-[11px] font-semibold text-emerald-200">
              Jordan phoneme debug
            </div>
            <div>audio.currentTime: {jordanPhonemeDebug.audioCurrentTime.toFixed(3)}</div>
            <div>lookAheadMs: {jordanPhonemeDebug.lookAheadMs.toFixed(0)}</div>
            <div>phonemeTimeline length: {jordanPhonemeDebug.phonemeTimelineLength}</div>
            <div>active phoneme: {jordanPhonemeDebug.activePhoneme ?? "none"}</div>
            <div>normalized phoneme: {jordanPhonemeDebug.normalizedPhoneme ?? "none"}</div>
            <div>active viseme: {jordanPhonemeDebug.activeViseme ?? "none"}</div>
            <div>
              active viseme target: {jordanPhonemeDebug.activeVisemeTargetValue.toFixed(3)}
            </div>
            <div>jawOpen: {jordanPhonemeDebug.jawOpenValue.toFixed(3)}</div>
            <div>fallback mode: {String(jordanPhonemeDebug.fallbackModeActive)}</div>
            <div>old mouth skipped: {String(jordanPhonemeDebug.oldMouthDriverSkipped)}</div>
          </div>
        )}
    </div>
  );
