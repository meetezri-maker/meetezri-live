...
const returnIdx = lines.findIndex(
  (l, i) => i > 600 && l.trim() === "return (" && lines[i + 1]?.includes("solace-login-page"),
);
...
if ([returnIdx, lotusIdx, mobileIdx].some((i) => i < 0)) {
  console.error("markers", { returnIdx, lotusIdx, mobileIdx });
  process.exit(1);
}
...
const form = formLines.map((l) => pad(18, l.trimStart())).join("\n");

const newReturn = `${pad(2, "return (")}
${pad(4, '<div className="solace-login-page flex h-dvh max-h-dvh flex-col overflow-hidden bg-[#0b0b1e] text-[#f4f4f8]">')}
...
${pad(8, "")}
${pad(8, "{/* Desktop */}")}
${pad(8, '<motion.div className="relative z-10 hidden h-full min-h-0 items-center gap-[clamp(24px,4vw,64px)] overflow-hidden px-[clamp(20px,3.5vw,56px)] lg:grid lg:grid-cols-[minmax(0,1.12fr)_minmax(420px,0.88fr)]">'.replace("motion.", ""))}
${pad(10, '<section className="flex h-full max-h-full min-h-0 flex-col justify-between py-1">')}
${pad(12, '<div className="min-h-0">')}
${pad(14, "<WelcomeBlock />")}
${pad(14, '<TrustStack className="mt-5" compact />')}
${pad(12, "</div>")}
${pad(12, '<EmotionalQuoteCard className="mt-3 shrink-0" />')}
${pad(10, "</section>")}
${pad(10, "")}
...
${pad(14, "<div")}
${pad(16, "className={cn(")}
${pad(18, '"relative max-h-[calc(100dvh-4.75rem-2rem)] overflow-y-auto overflow-x-hidden rounded-[2rem] border border-[#d81b60]/28",')}
${pad(18, '"bg-[rgba(11,11,30,0.44)] p-[clamp(1.35rem,2.6vw,2.1rem)] shadow-[0_0_0_1px_rgba(168,85,247,0.14),0_56px_140px_-52px_rgba(0,0,0,0.9),inset_0_1px_0_rgba(255,255,255,0.09)] backdrop-blur-[26px]",')}
${pad(18, '"supports-[backdrop-filter]:bg-[rgba(11,11,30,0.36)] [scrollbar-width:thin]",')}
${pad(16, ")}")}
${pad(14, ">")}
${pad(16, '<motion.div className="pointer-events-none absolute inset-0 rounded-[2rem] bg-gradient-to-br from-[#E91E63]/[0.08] via-transparent to-[#9C27B0]/[0.1]" />'.replace("motion.", ""))}
${pad(16, '<div className="pointer-events-none absolute -right-[15%] top-[-5%] h-[50%] w-[75%] rounded-full bg-[radial-gradient(circle,rgba(233,30,99,0.14),transparent_68%)]" />')}
${pad(16, '<div className="relative">')}
${form}
${pad(16, "</div>")}
${pad(14, "</div>")}
${pad(14, '<p className="mt-3 flex justify-center gap-2 text-[11px] leading-relaxed text-violet-200/42">')}
...
${pad(8, "")}
${pad(8, "{/* Mobile */}")}
${pad(8, '<div className="relative z-10 flex h-full min-h-0 flex-col overflow-y-auto lg:hidden">')}
${pad(10, '<div className="min-h-[min(38svh,20rem)] shrink-0" aria-hidden />')}
${pad(10, '<div className="px-5 pb-5 pt-1">')}
${pad(12, "<WelcomeBlock />")}
${pad(12, '<EmotionalQuoteCard className="mx-auto mt-5 max-w-md" />')}
${pad(10, "</motion.div>".replace("motion.", ""))}
${pad(10, '<div className="px-5 pb-8">')}
...
${pad(12, "</div>")}
${pad(10, "</div>")}
${pad(10, '<div className="border-t border-white/[0.05] bg-[#080812]/40 px-5 py-8 backdrop-blur-md">')}
...
const fixed = newReturn.replace(/<motion\.div/g, "<div").replace(/<\/motion\.div>/g, "</div>");

const endIdx = lines.findIndex((l, i) => i > returnIdx && l.trim() === ");");
const out = [...lines.slice(0, returnIdx), fixed, ...lines.slice(endIdx + 1)];
fs.writeFileSync(p, out.join("\n"));
console.log("return replaced", returnIdx, endIdx);
