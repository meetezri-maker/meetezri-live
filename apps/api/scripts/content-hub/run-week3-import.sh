#!/usr/bin/env bash
#
# Runs the Week 3 importer against the SESSION pooler.
#
...
#
# Usage:
#   scripts/content-hub/run-week3-import.sh [--apply --confirm-production --actor=<id>]
...
exec npx ts-node-dev --transpile-only --respawn=false scripts/content-hub/import-week3.ts "$@"
