/**
 * Week 4 Content Hub import — a ONE-TIME, CONTROLLED migration utility.
 *
 ...
 *     npx ts-node-dev --transpile-only --respawn=false scripts/content-hub/import-week4.ts
 *
 *   Apply:
 *     … scripts/content-hub/import-week4.ts --apply --confirm-production --actor=<profile-id>
 *
 * TWO PASSES, because the Week 4 link graph is cyclic (A→B, A→G). No single-pass import can
...
import {
  ...
  type Week4Asset,
} from '../../src/modules/content-hub/week4/week4-content';
import {
  decideAction,
  validateAsset,
  type ExistingRow,
...
function countFaqItems(asset: Week4Asset): number {
...
function countWords(asset: Week4Asset): number {
  const text: string[] = [];
...
function blockTypeSummary(asset: Week4Asset): string {
...
async function main() {
  ...
  line('═'.repeat(78));
  line('  SOLACE CONTENT HUB — WEEK 4 IMPORT');
  line('═'.repeat(78));
  line(`  mode          : ${mode}`);
  line(`  host          : ${target.host}`);
  line(`  database      : ${target.database}`);
  line(`  supabase ref  : ${target.project}`);
  line('  source        : SOLACE MONTH 1 WEEK 4 OPERATIONAL WORKBOOK.docx (W4-B001, W4-A001; W4-G001 deferred)');
  line('═'.repeat(78));
   ...
   * `--actor` is WHO RAN THE IMPORT. `--author` is the CONTENT'S BYLINE. The Week 4 workbook
  ...
  // ── Asset summary ─────────────────────────────────────────────────────────
  line('\n─── ASSETS ───');
  for (const asset of WEEK4_ASSETS) {
    ...
    if (decision.action === 'create') {
      ...
      await updateContent(
        created.id,
        {
          ...
          changeSummary: 'Week 4 workbook import',
        } as never,
        actor!,
      );
      ...
      await updateContent(
        existing!.id,
        {
          ...
          expectedUpdatedAt: fresh.updated_at.toISOString(),
          createRevision: false,
          changeSummary: 'Week 4 workbook re-import',
        } as never,
        actor!,
      );
