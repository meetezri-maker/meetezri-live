#!/usr/bin/env bash
#
# Runs the Month 2 Week 1 importer against the SESSION pooler.
#
...
#
# Usage:
#   scripts/content-hub/run-m2w1-import.sh [--apply --confirm-production --actor=<id>]
...
exec npx ts-node-dev --transpile-only --respawn=false scripts/content-hub/import-m2w1.ts "$@"
