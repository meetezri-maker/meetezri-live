/**
 * Month 2 Week 1 Content Hub import — a ONE-TIME, CONTROLLED migration utility.
 *
 ...
 *     npx ts-node-dev --transpile-only --respawn=false scripts/content-hub/import-m2w1.ts
 *
 *   Apply:
 *     … scripts/content-hub/import-m2w1.ts --apply --confirm-production --actor=<profile-id>
 *
 * TWO PASSES, because the Month 2 Week 1 link graph is cyclic (A→B, A→G). No single-pass import can
...
import {
  ...
  type M2W1Asset,
} from '../../src/modules/content-hub/month2week1/m2w1-content';
import {
  decideAction,
  validateAsset,
  type ExistingRow,
...
function countFaqItems(asset: M2W1Asset): number {
...
function countWords(asset: M2W1Asset): number {
  const text: string[] = [];
...
function blockTypeSummary(asset: M2W1Asset): string {
...
async function main() {
  ...
  line('═'.repeat(78));
  line('  SOLACE CONTENT HUB — MONTH 2 WEEK 1 IMPORT');
  line('═'.repeat(78));
  line(`  mode          : ${mode}`);
  line(`  host          : ${target.host}`);
  line(`  database      : ${target.database}`);
  line(`  supabase ref  : ${target.project}`);
  line('  source        : SOLACE MONTH 1 MONTH 2 WEEK 1 OPERATIONAL WORKBOOK.docx (M2-W1-B001, M2-W1-A001; M2-W1-G001 deferred)');
  line('═'.repeat(78));
    ...
    if (decision.action === 'create') {
      ...
      await updateContent(
        created.id,
        {
          ...
          changeSummary: 'Month 2 Week 1 workbook import',
        } as never,
        actor!,
      );
      ...
      await updateContent(
        existing!.id,
        {
          ...
          expectedUpdatedAt: fresh.updated_at.toISOString(),
          createRevision: false,
          changeSummary: 'Month 2 Week 1 workbook re-import',
        } as never,
        actor!,
      );
