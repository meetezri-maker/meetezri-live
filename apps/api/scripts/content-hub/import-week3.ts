/**
 * Week 3 Content Hub import — a ONE-TIME, CONTROLLED migration utility.
 *
 ...
 *     npx ts-node-dev --transpile-only --respawn=false scripts/content-hub/import-week3.ts
 *
 *   Apply:
 *     … scripts/content-hub/import-week3.ts --apply --confirm-production --actor=<profile-id>
 *
 * TWO PASSES, because the Week 3 link graph is cyclic (A→B, A→G). No single-pass import can
...
import {
  ...
  type Week3Asset,
} from '../../src/modules/content-hub/week3/week3-content';
import {
  decideAction,
  validateAsset,
  type ExistingRow,
...
function countFaqItems(asset: Week3Asset): number {
...
function countWords(asset: Week3Asset): number {
  const text: string[] = [];
...
function blockTypeSummary(asset: Week3Asset): string {
...
async function main() {
  ...
  line('═'.repeat(78));
  line('  SOLACE CONTENT HUB — WEEK 3 IMPORT');
  line('═'.repeat(78));
  line(`  mode          : ${mode}`);
  line(`  host          : ${target.host}`);
  line(`  database      : ${target.database}`);
  line(`  supabase ref  : ${target.project}`);
  line('  source        : WEEK 3 OPERATIONAL WORKBOOK.docx (W3-B001, W3-G001, W3-A001)');
  line('═'.repeat(78));
   ...
   * `--actor` is WHO RAN THE IMPORT. `--author` is the CONTENT'S BYLINE. The Week 3 workbook
  ...
  // ── Asset summary ─────────────────────────────────────────────────────────
  line('\n─── ASSETS ───');
  for (const asset of WEEK3_ASSETS) {
    ...
    if (decision.action === 'create') {
      ...
      await updateContent(
        created.id,
        {
          ...
          changeSummary: 'Week 3 workbook import',
        } as never,
        actor!,
      );
      ...
      await updateContent(
        existing!.id,
        {
          ...
          expectedUpdatedAt: fresh.updated_at.toISOString(),
          createRevision: false,
          changeSummary: 'Week 3 workbook re-import',
        } as never,
        actor!,
      );
